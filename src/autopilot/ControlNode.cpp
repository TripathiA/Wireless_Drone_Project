	...
	while(currentKI == NULL && commandQueue.size() > 0)
	{
		...
		else if(sscanf(command.c_str(),"goto %f %f %f %f",&parameters[0], &parameters[1], &parameters[2], &parameters[3]) == 4)
		{
			//parameters[0] = 1;
                        //parameters[1] = 0;
			//parameters[2] = 0;
                        publishCommand("u l Changed values 0 0");
...
void ControlNode::comCb(const std_msgs::StringConstPtr str)
{
	...
	//if(bad_quality)
	//{
	//	std::string cmd = "c moveBy 1 0 0 0";
	//	commandQueue.push_back(cmd);
	//	bad_quality = false;
	//}
...
//void ControlNode::check_channel_quality()
//{
//	int i = 0;
//	publishCommand("u l Autopilot: BAD QUALITYsjbj");
//	ROS_INFO("BAD CHANNEL QUALOTY! MOVE!");
//	if(i == 10)
//	{
//		//bad_quality = true;
//		i = 0;
...
void ControlNode::Loop()
{
	...
	publishCommand("u l Autopilot: BAD QUALITY OUT");
	ROS_INFO("BAD CHANNEL QUALOTY! MOVE!!");
	//std::thread check_channel(check_channel_quality);
	//pthread_create(&check_channel,NULL,check_channel_quality,this);
